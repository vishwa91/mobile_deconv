...
static image<uchar> *loadPBM(char *name) {
  char buf[BUF_SIZE];
...
static void savePBM(image<uchar> *im, char *name) {
  int width = im->width();
...
static image<uchar> *loadPGM(char *name) {
  char buf[BUF_SIZE];
...
static void savePGM(image<uchar> *im, char *name) {
  int width = im->width();
  int height = im->height();
  std::ofstream file(name, std::ios::out | std::ios::binary);
...
static image<rgb> *loadPPM(char *name) {
...
static void savePPM(image<rgb> *im, char *name) {
  int width = im->width();
  int height = im->height();
  std::ofstream file(name, std::ios::out | std::ios::binary);
...
template <class T>
void load_image(image<T> **im, char *name) {
  char buf[BUF_SIZE];
...
template <class T>
void save_image(image<T> *im, char *name) {
  int width = im->width();
  int height = im->height();
  std::ofstream file(name, std::ios::out | std::ios::binary);
