...
def gauss_pdf(x, mean, var):
    ...
    mul = 1/sqrt(2*pi*var)
    index = -((x-mean)**2)/(2*var)
    return mul*exp(index)
...
if __name__ == '__main__':
    ...
    valx, lenx, _ = hist(dhandle.xaccel, bins=nbins, normed=True)
    valy, leny, _ = hist(dhandle.yaccel, bins=nbins, normed=True)
    valz, lenz, _ = hist(dhandle.zaccel, bins=nbins, normed=True)
    '''
    xmean = mean(dhandle.xaccel); xvar = var(dhandle.xaccel)
    ymean = mean(dhandle.yaccel); yvar = var(dhandle.yaccel)
    zmean = mean(dhandle.zaccel); zvar = var(dhandle.zaccel)
    '''
    x = arange(-2, 2, 0.0001)
    
    plot(x, gauss_pdf(x, xmean, xvar))
    plot(x, gauss_pdf(x, ymean, yvar))
    plot(x, gauss_pdf(x, zmean, zvar))
    show()
    '''
    f = open('accel_stats.dat', 'w')
    f.write('xmean: %f\nymean: %f\nzmean: %f\n'%(xmean, ymean,zmean))
    f.write('xvar: %f\nyvar: %f\nzvar: %f'%(xvar, yvar, zvar))
    f.close()
    #dhandle.plot_position()
